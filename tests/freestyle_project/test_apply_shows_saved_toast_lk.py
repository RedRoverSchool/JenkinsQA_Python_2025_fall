...
def test_tc_02_001_006_apply_shows_saved_toast(page):
    ...
    apply_btn_loc = "button[name='Apply']"

    notification_bar_loc = "#notification-bar"
    ...
    page.locator(apply_btn_loc).click()

    bar = page.locator(notification_bar_loc)

    expect(bar).to_be_visible()  # 1) Notification bar стал видимым

    expect(bar).to_contain_text("Saved")  # 2) Notification bar содержит текст "Saved"

    expect(bar).not_to_have_class(re.compile("jenkins-notification--hidden"))  # 3) Класс hidden исчез
    # (notification активно показывается)
